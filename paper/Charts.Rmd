...
ffa_chronics <- ggplot(df_chronic_ffa, aes(x=rep_date_date , y=perc_chronis )) +
   ...
   theme_few() +
  labs(title = "Proportions of treated patients with chronic diseases",
       x = " ", y = "Percent",
...
kable(df_geda_theme, align = "c", booktabs = TRUE, escape = FALSE, "latex") %>%
...
df_barchart_group_ranking <- df_barchart_group_ranking %>% dplyr::rename('Variable' = `_changed`)

ggplot(df_barchart_group_ranking, aes(rn, count)) + geom_bar(aes(fill = Variable), 
