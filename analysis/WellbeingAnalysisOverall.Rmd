...
library(sandwich) #for stargazer
library(ivpack)
...
summary(psm_results)
stargazer(psm_results)
```


2) Instrumental Variable
```{r}
#csv file was read with mapping of IV
iv <- read.csv("data/IV_variable.csv")
#iv <- read.csv("data/IV_variable_BF_2.csv")

#join the instrumental variables to the data
df_iv <- merge(x = df_base, y = iv, all.x = TRUE, by.x = "bula", by.y = "bula")

#summary(df_iv)

#Before multiplying the variables need to convert from factor to numeric
df_iv$IPinfl6B_num <- as.numeric(as.character(df_iv$IPinfl6B))

#NEED TO CONVERT THE NUMERIC VARIABLES TO 1 -> 0, 2 -> 1.5, 3 -> 3.5 AND 4 -> 5 given they represent a range of values...we are using the average
df_iv$IPinfl6B_num[df_iv$IPinfl6B_num == 1] <- 0
df_iv$IPinfl6B_num[df_iv$IPinfl6B_num == 2] <- 1.5
df_iv$IPinfl6B_num[df_iv$IPinfl6B_num == 3] <- 3.5
df_iv$IPinfl6B_num[df_iv$IPinfl6B_num == 4] <- 5

#multiplying the variables together
df_iv$IV <- df_iv$st_km * df_iv$IPinfl6B_num

#this variable is using only the density of streets / km2 variable as an IV. Only used to validate that state level IV would not be appropriate.
#df_iv$IV <- df_iv$st_km 

#BILL FLUNEY Variable
#df_iv$IV <- df_iv$fatal_acc_per_million


#Graph the histogram of IV variables by having or not gone to visit a general practicioner
df_iv %>%
  mutate(IAarzt1B = ifelse(IAarzt1B == 1, labs[1], labs[2])) %>%
  ggplot(aes(x = IV)) +
  geom_histogram(color = "white") +
  facet_wrap(~IAarzt1B) +
  xlab("Instrumental Variable") +
  theme_bw()
...
#First Stage
Looking at the relationship between the IV and a person visiting a general practictioner
```{r}
#Convert from Factor to Numeric in order to run IVREG
df_iv$GZmehm1 <- as.numeric(as.character(df_iv$GZmehm1))

first_stage <- lm(IAarzt1B ~ IV + sex + age5B + MIbirthplace + MIcitizen + bula + PAgroe + PAgewiB + IAkv1D + SDaeqeink_k + SDbild4C + SDbverh2 + SDhvtypD + SDmainstat + SDalo+ PAhhtype + AKoft12_k + RCstat + KAehispaq2 + KAspodauC_k + KAehispaq8, data = df_iv)
...
#Second Stage
```{r}
iv1 = ivreg(GZmehm1 ~ IAarzt1B + sex + age5B + MIbirthplace + MIcitizen + bula + PAgroe + PAgewiB + IAkv1D + SDaeqeink_k + SDbild4C + SDbverh2 + SDhvtypD + SDmainstat + SDalo+ PAhhtype + AKoft12_k + RCstat + KAehispaq2 + KAspodauC_k + KAehispaq8
            |
              sex + age5B + MIbirthplace + MIcitizen + bula + PAgroe + PAgewiB + IAkv1D + SDaeqeink_k + SDbild4C + SDbverh2 + SDhvtypD + SDmainstat + SDalo+ PAhhtype + AKoft12_k + RCstat + KAehispaq2 + KAspodauC_k + KAehispaq8 + IV, data = df_iv)
