...
hist(as.numeric(df_geda_num[!is.na(df_geda_num$GZmehm1),]$GZmehm1), breaks=5, main="Histogram of the wellbeing score (response)", xlab="1=very bad | 5=very good")
```
...
Encode number of GP visited in the last 12 months --> if -96, then 0 
...
mlm <- multinom(GZmehm1 ~ sex+age5B+bula+PAgroe+KHhyp, data = trainingData)
# 
# mlm <- nnet(GZmehm1 ~ sex+age5B+bula+PAgroe+KHhyp, data = trainingData,family="multinomial",size = 5574900,MaxNWts =10000000)
m_sample <- polr(GZmehm1 ~ sex+age5B+bula+PAgroe+KHhyp, data = trainingData, Hess=TRUE)
